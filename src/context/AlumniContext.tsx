...
const AlumniProvider = (props: Props) => {
  ...
  // const [listMember, setMember] = useState<IUser[]>([])
  ...
  const fetchAlumnis = async (payload: { take: number; search?: any ;status?:any}) => {
    ...
    const fetchMember = async (payload: { take: number; search?: any; status?: any ;id?:any}) => {
      // only trigger in page 1
      ...
      try {
        const response = await HttpClient.get(AppConfig.baseUrl + '/alumni/getMember', {
          page: page,
          ...payload
        })
  ...
  const values = useMemo(
    ...
    [page, totalAlumni, setPage, listAlumni, onLoading, hasNextPage, fetchAlumnis,fetchMember]
  )
