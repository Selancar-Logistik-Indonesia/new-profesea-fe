...
export default function DialogCalculateAllUserCP(props: IDialogCalculateAllUserPointProps) {
  ...
  const calculateEachUser = () => {
    return usersId.map(async (userId: number) => {
      try {
        const calculate = await HttpClient.get(`/public/data/user/calculate-percentage/${userId}`)
  ...
  return (
    <Dialog open={props.visible} scroll='paper' maxWidth={'md'} fullWidth>
      <DialogTitle id='scroll-dialog-title'>
        <Grid container>
          <Grid item width={'50%'}>
            User Calculation Completion Percentage
          </Grid>
          <Grid item width={'50%'}>
                ...
                itemResult?.map((item: any, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        {item?.detail_percentage
                          ? Object.keys(item?.detail_percentage).map(function (key) {
                              return (
                                <div key={key}>
                                  {key} : {item?.detail_percentage[key]}
                                </div>
                              )
                            })
                          : JSON.stringify(item?.detail_percentage)}
                      </td>
                    </tr>
                  )
                })
