  ...
  const getListJob = async () => {
    try {
      const resp = await HttpClient.get(
        `/job?search=${search}&page=${page}&take=${perPage}&category_id=${filterJC}&rolelevel_id=${filterRL}&roletype_id=${filterJT}`
      )
      if (resp.status != 200) {
              ...
              <Grid container justifyContent='flex-start'>
                ...
                <Grid item>
                  <Autocomplete
                    ...
                    onChange={(event: any, newValue: JobCategory | null) => (newValue ? setJC(newValue.id) : setJC(0))}
                  />
                </Grid>
                <Grid item>
                  <Autocomplete
