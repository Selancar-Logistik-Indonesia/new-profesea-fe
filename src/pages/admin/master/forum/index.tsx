...
const ForumScreen = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<Forum | null>(null);
    ...
    const getListForum = async () => {
        try {
            const resp = await HttpClient.get(`/forum?search=${search}&page=${page}&take=${perPage}`);
            if (resp.status != 200) {
                throw resp.data.message ?? "Something went wrong!";
            }

            const rows = resp.data.forums.data as Forum[];
            ...
            setRowCount(resp?.data?.forums?.total ?? 0);
            setDataSheet(items);
        } catch (error) {
    ...
    const deleteHandler = (row: Forum) => {
        setSelectedItem(row);
        setOpenDelModal(true);
    }

    const updateHandler = (row: Forum) => {
        setSelectedItem(row);
