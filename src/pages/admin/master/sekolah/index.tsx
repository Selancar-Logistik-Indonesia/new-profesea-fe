...
import SekolahDatagrid, { RowItem } from './SekolahDatagrid';
...
const SekolahScreen = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<SekolahType | null>(null);
    ...
    const getListSekolah = async () => {
        try {
            const resp = await HttpClient.get(`/sekolah?search=${search}&page=${page}&take=${perPage}`);
            if (resp.status != 200) {
                throw resp.data.message ?? "Something went wrong!";
            }

            const rows = resp.data.sekolah.data as SekolahType[];
            ...
            setRowCount(resp?.data?.sekolah?.total ?? 0);
            setDataSheet(items);
        } catch (error) {
    ...
    const deleteHandler = (row: SekolahType) => {
      setSelectedItem(row)
      setOpenDelModal(true)
    }

    const updateHandler = (row: SekolahType) => {
      setSelectedItem(row)
