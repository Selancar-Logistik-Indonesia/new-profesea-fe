...
import VesselDatagrid, { RowItem } from './VesselDatagrid';
...
const VesselTypeScreen = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<VesselType | null>(null);
    ...
    const getListVesselType = async () => {
        try {
            const resp = await HttpClient.get(`/vessel-types?search=${search}&page=${page}&take=${perPage}`);
            if (resp.status != 200) {
                throw resp.data.message ?? "Something went wrong!";
            }

            const rows = resp.data.VesselTypes.data as VesselType[];
            ...
            setRowCount(resp?.data?.VesselTypes?.total ?? 0);
            setDataSheet(items);
        } catch (error) {
    ...
    const deleteHandler = (row: VesselType) => {
        setSelectedItem(row);
        setOpenDelModal(true);
    }

    const updateHandler = (row: VesselType) => {
        setSelectedItem(row);
...
VesselTypeScreen.acl = {
    action: 'read',
    subject: 'master/vessel-type'
};

export default VesselTypeScreen
