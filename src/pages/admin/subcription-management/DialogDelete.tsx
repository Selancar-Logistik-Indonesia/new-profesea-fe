...
type DeleteDialogProps = {
    selectedItem: TransactionSubscription;
    visible: boolean;
...
const DialogDelete = (props: DeleteDialogProps) => {
    ...
    const handleDelete = async () => {
        try {
            const resp = await HttpClient.del(`/user-subscription/${props.selectedItem.id}`);
            if (resp.status != 200) {
                throw resp.data.message ?? "Something went wrong!";
            }

            props.onCloseClick();
            toast.success(`${props.selectedItem.user.name} deleted successfully!`);
        } catch (error) {
            console.error(error)
        }
        ...
        <Dialog
            ...
            >
                ...
                <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant='body2'>Are you sure delete {props.selectedItem.user.name}?</Typography>
                </Box>
            </DialogContent>
            <DialogActions
                sx={{
