...
const DialogProfileDeleteConfirmation = (props: IProps) => {
  ...
  const handleProfileDelete = () => {
    HttpClient.post(AppConfig.baseUrl + '/user/friend-request/accept', {
      ...
      () => {
        toast.error(' FAILED Accepting friend request ')
        props.onCloseClick()
        setOnLoading(false)
      }
    )
      ...
      >
        ...
        <Box sx={{ mb: 3, textAlign: 'center' }}>
          <Typography variant='body2' color={'#32487A'} fontWeight='600' fontSize={18}>
            Are you sure want to Accepting <b>{props?.selectedItem?.name}</b> friend request ?
          </Typography>
        </Box>
        <div></div>
      </DialogContent>
