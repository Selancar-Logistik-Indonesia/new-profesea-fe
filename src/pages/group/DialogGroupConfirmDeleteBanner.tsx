...
const DialogGroupConfirmDeleteBanner = (props: IProps) => {
  const [onLoading, setOnLoading] = useState(false)

  const handleBannerDelete = () => {
    HttpClient.del(AppConfig.baseUrl + `/group/${props.id}/delete-banner`).then(
      () => {
      ...
      >
        ...
        <Box sx={{ mb: 3, textAlign: 'center' }}>
          <Typography variant='body2' color={'#32487A'} fontWeight='600' fontSize={18}>
            Are you sure want to delete your Banner Picture?
          </Typography>
        </Box>
      </DialogContent>
      <DialogActions
...
export default DialogGroupConfirmDeleteBanner
