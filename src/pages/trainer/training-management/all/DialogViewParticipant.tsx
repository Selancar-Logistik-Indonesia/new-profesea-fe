...
import { getCleanErrorMessage, getUserAvatar, toLinkCase } from 'src/utils/helpers'
...
const DialogViewParticipant = (props: ViewProps) => {
  ...
  const getParticipants = async () => {
    ...
    try {
      ...
      const { participants } = response.data as {
        participants: { data: ITrainingParticipant[]; total: number; next_page_url: string }
      }
      ...
      >
        ...
        <Box sx={{ mb: 6 }}>
          ...
          <Typography variant='body2' sx={{ mb: 3 }}>
            Total Participants: {training.count_participant}
          </Typography>
        </Box>
            ...
            trainingUsers.map(e => (
              <Box sx={{ display: 'flex', flexDirection: 'row', width: '100%', mb: 4 }} key={e.id}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'start', justifyContent: 'center' }}>
                  ...
                  <Typography lineHeight={1.4} variant='caption' fontSize={10}>
                    {e.user.email}
                  </Typography>
                </Box>
