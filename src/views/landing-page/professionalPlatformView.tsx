import { Icon } from '@iconify/react'
import { Avatar, Box, Divider, Grid, IconButton, Paper, Typography } from '@mui/material'
import Link from 'next/link'
import { useEffect, useState } from 'react'
import Job from 'src/contract/models/job'
import { HttpClient } from 'src/services'
import { timeCreated } from 'src/utils/helpers'
import { format } from 'date-fns'

const renderSalary = (salaryStart: any, salaryEnd: any, currency: string) => {
  if (salaryStart == 0) {
    return '-'
  }

  if (salaryStart && salaryEnd) {
    if (currency == 'IDR') {
      if (salaryEnd == 0) {
        return `${salaryStart.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')} (${currency})`
      } else {
        return `${
          salaryStart.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') +
          ' - ' +
          salaryEnd.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')
        } (${currency})`
      }
    } else {
      if (salaryEnd == 0) {
        return `${salaryStart.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')} (${currency})`
      } else {
        return `${salaryStart.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')} - ${salaryEnd
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, '.')} (${currency})`
      }
    }
  } else {
    return '-'
  }
}

const JobCard = ({ jobs }: { jobs: Job[] | null }) => {
  return (
    <Grid container sx={{ display: 'flex', flexWrap: 'nowrap', justifyContent: 'space-between', gap: '17px' }}>
      <Grid item container spacing={4}>
        {jobs &&
          jobs.map((job, i) => {
            const userPhoto = job.company?.photo ? job.company?.photo : '/images/avatars/default-user.png'
            const companyNameUrl = job.company.name.toLowerCase().split(' ').join('-')
            const jobTitleUrl = job.job_title ? job.job_title?.toLowerCase().split(' ').join('-') : ''
            const link = `/job/${companyNameUrl}/${job.id}/${jobTitleUrl}`

            return (
              <Grid item key={i} xs={12} md={3} component={Link} href={link}>
                <Paper
                  sx={{
                    padding: '12px',
                    borderRadius: '8px',
                    width: '100%',
                    height: '100%',
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '6px',
                    transition: '1s',
                    '&:hover': { boxShadow: 3, transform: 'scale(1.02)' }
                  }}
                >
                  <Box
                    sx={{
                      display: 'flex',
                      flexDirection: 'column',
                      justifyContent: 'flex-start',
                      gap: '10px',
                      flexGrow: 1
                    }}
                  >
                    <Typography
                      sx={{
                        color: '#303030',
                        fontSize: 12,
                        fontWeight: 700,
                        textTransform: 'capitalize',
                        whiteSpace: 'nowrap',
                        overflow: 'hidden',
                        textOverflow: 'ellipsis'
                      }}
                    >
                      {job.job_title ?? '-'}
                    </Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <Avatar src={userPhoto} alt='profile-picture' sx={{ width: 34, height: 34 }} />
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <Typography sx={{ color: '#2D3436', fontSize: 10, fontWeight: 700 }}>
                          {job.company.name}
                        </Typography>
                        <Typography sx={{ color: '#868686', fontSize: 10, fontWeight: 400 }}>
                          {job.city.city_name}, {job.country.nicename}
                        </Typography>
                      </Box>
                    </Box>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '8px', flexGrow: 1 }}>
                      <Box
                        sx={{
                          height: 'fit-content',
                          padding: '4px 8px',
                          border: '1px solid #32497A',
                          borderRadius: '4px'
                        }}
                      >
                        <Typography sx={{ color: '#32497A', fontSize: 10, fontWeight: 400, whiteSpace: 'nowrap' }}>
                          {job.role_type?.name}
                        </Typography>
                      </Box>
                      <Box
                        sx={{
                          height: 'fit-content',
                          padding: '4px 8px',
                          border: '1px solid #32497A',
                          borderRadius: '4px'
                        }}
                      >
                        <Typography sx={{ color: '#32497A', fontSize: 10, fontWeight: 400, whiteSpace: 'nowrap' }}>
                          {job.vessel_type.name}
                        </Typography>
                      </Box>
                    </Box>
                    <Typography sx={{ color: 'black', fontSize: 10, fontWeight: 400 }}>
                      {renderSalary(job.salary_start, job.salary_end, job.currency as string)}
                    </Typography>
                    <Typography sx={{ color: '#525252', fontSize: 10, fontWeight: 400 }}>
                      Onboarding on{' '}
                      <span style={{ color: '#32497A', fontWeight: 700 }}>
                        {format(new Date(job.onboard_at), 'dd MMMM yyyy') ?? '-'}
                      </span>
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <Divider sx={{ border: '1px solid #F0F0F0' }} />
                    <Typography sx={{ color: '#868686', fontSize: 10, fontWeight: 400 }} fontSize={12}>
                      {job.created_at ? timeCreated(job.created_at) : '-'}
                    </Typography>
                  </Box>
                </Paper>
              </Grid>
            )
          })}
      </Grid>
      <Box
        sx={{
          flexShrink: 0,
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'center',
          alignItems: 'center',
          gap: '12px'
        }}
      >
        <IconButton sx={{ backgroundColor: 'rgba(10, 12, 15, 0.5)', borderRadius: '200px' }}>
          <Icon icon='mdi:chevron-right' color='white' fontSize={34} />
        </IconButton>
        <Typography sx={{ color: 'white', fontSize: '14px', fontWeight: 400, textAlign: 'center' }}>
          Discover <br />
          More Jobs
        </Typography>
      </Box>
    </Grid>
  )
}

const ProfessionalPlatformView = () => {
  const [jobs, setJobs] = useState<Job[] | null>(null)
  //   const [onLoading, setOnLoading] = useState(false)

  const fetchJobs = async () => {
    // setOnLoading(true)
    try {
      const resp = await HttpClient.get(`/public/data/job`, {
        page: 1,
        take: 4,
        employee_type: 'onship'
      })

      if (resp.status == 200) {
        const data = resp.data.jobs.data
        setJobs(data)
      }
    } catch (error) {
      console.error(error)
    }

    // setOnLoading(false)
  }

  useEffect(() => {
    fetchJobs()
  }, [])

  return (
    <Grid
      container
      sx={{
        backgroundImage: 'url(/images/professional-platform-banner.png)',
        backgroundSize: 'cover',
        backgroundPosition: '100% 20%',
        height: '622px',
        borderRadius: '20px',
        backgroundColor: 'gray',
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'flex-end',
        overflow: 'hidden'
      }}
    >
      <Grid item container sx={{ mb: '24px', display: 'flex', justifyContent: 'flex-start' }}>
        <Box sx={{ maxWidth: '560px', ml: '24px' }}>
          <Typography sx={{ mb: '12px', color: 'white', fontSize: 40, fontWeight: 700 }}>
            Profesea for Profesionnal
          </Typography>
          <Typography sx={{ color: 'white', fontSize: 16, fontWeight: 400, lineHeight: '21px' }}>
            Build your dream team with our global hiring solution for the maritime and logistics industry. We help you
            easily recruit top professionals while ensuring compliance with local regulations—so you can expand your
            operations seamlessly across the globe.
          </Typography>
        </Box>
      </Grid>
      <Grid
        item
        container
        sx={{ p: '24px', backgroundColor: 'rgba(134, 134, 134, 0.30)', backdropFilter: 'blur(10px)' }}
      >
        <JobCard jobs={jobs} />
      </Grid>
    </Grid>
  )
}

export default ProfessionalPlatformView
