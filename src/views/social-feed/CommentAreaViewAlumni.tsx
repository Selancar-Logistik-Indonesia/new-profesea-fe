...
import SubCommentAreaViewAlumni from './SubCommentAreaViewAlumni'
...
const CommentCard = (props: { comment: ISocialFeedComment }) => {
  ...
  return (
    <Box key={comment.id} sx={{ display: 'flex', flexDirection: 'column', mt: 5 }}>
      ...
      {openReply && <SubCommentAreaViewAlumni key={comment.id} item={comment} />}
    </Box>
  )
}

const CommentAreaViewAlumni = (props: { item: ISocialFeed }) => {
  const { item } = props
  const [onLoading, setOnLoading] = useState(true)
  const { getComments, commentSignature } = useAlumniFeed()
...
export default CommentAreaViewAlumni
