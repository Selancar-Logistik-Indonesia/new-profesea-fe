...
const SubCommentCardGroup = (props: { comment: ISocialFeedComment }) => {
    const { comment } = props;
...
const SubCommentAreaViewGroup = (props: { item: ISocialFeedComment }) => {
    const { item } = props;
    const [onLoading, setOnLoading] = useState(true);
    const { getComments, subCommentSignature } = useGroupFeed();
    ...
    return (
        <Box component='div' sx={{ ml: 5 }}>
            {commentObj?.data && commentObj?.data.length > 0 && (
                <Box mb={5}>
                    {commentObj?.data.reverse().map(comment => (<SubCommentCardGroup key={comment.id} comment={comment} />))}
                </Box>
            )}
            ...
            <CommentFormGroup feedId={item.id} replyable_type='comment' />
        </Box>
    );
}

export default SubCommentAreaViewGroup;